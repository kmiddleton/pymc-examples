---
title: 'GLM-missing-values-in-covariates'
...
# uv run ipython kernel install --user --env VIRTUAL_ENV $(pwd)/.venv --name="pymc-examples"
...
KWS_PNT = dict(
    estimator=np.mean, errorbar=("ci", 94), join=False, color="#32CD32"
)
...
### Create "true" (unobserved) dataset (without missing values)
...
dfraw = pd.DataFrame(
    {
        "a": RNG.normal(
            loc=REFVAL_X_MU["a"], scale=REFVAL_X_SIGMA["a"], size=N
        ),
        "b": RNG.normal(
            loc=REFVAL_X_MU["b"], scale=REFVAL_X_SIGMA["b"], size=N
        ),
        "c": RNG.normal(
            loc=REFVAL_X_MU["c"], scale=REFVAL_X_SIGMA["c"], size=N
        ),
        "d": RNG.normal(
            loc=REFVAL_X_MU["d"], scale=REFVAL_X_SIGMA["d"], size=N
        ),
    },
...
### Force 2 features to contain 40% unobserved missing values

```{python}
df = dfraw.copy()

prop_missing = 0.4
df.loc[RNG.choice(df.index, int(N * prop_missing), replace=False), "c"] = (
    np.nan
)
df.loc[RNG.choice(df.index, int(N * prop_missing), replace=False), "d"] = (
    np.nan
)
...
display(
    pd.concat(
        (df.describe(include="all").T, df.isnull().sum(), df.dtypes), axis=1
    )
)
```
...
> NOTE we don't need any further steps to prepare the dataset (e.g., clean observations & features, force datatypes, set
> indexes, etc), so we will move straight to EDA and transformations for model input

## Very limited quick EDA
...
### Bivariate: target vs predictors
...
## Transform dataset to `dfx` for model input
...
### Separate `df_train` and `df_holdout`
...
```{python}
df_holdout = (
    df.loc[list(set(df.index.values) - set(df_train.index.values))]
    .copy()
    .sort_index()
)
...
display(
    pd.concat(
        (
            df_holdout.describe(include="all").T,
...
# `Model0`: Baseline without Missing Values
...
## Quickly prepare non-missing datasets based on `dfraw`
...
## Sample Prior Predictive, View Diagnostics
...
### In-Sample Prior PPC (Retrodictive Check)
...
## Sample Posterior, View Diagnostics

### Sample Posterior and PPC
...
f = plot_traces_and_display_summary(
    id0, rvs=RVS_PRIOR, mdlnm="mdl0", energy=True
)
```
...
### In-Sample Posterior PPC (Retrodictive Check)
...
def plot_forest(
    ...
    _ = f.suptitle(
        f"Forestplot of {grp.title()} level values for `{rvs}` - `{mdlnm}`"
    )
    _ = f.tight_layout()
    return f
...
## Create PPC Forecast on `dfrawx_holdout` set

### Replace dataset with `dfrawx_holdout`
...
### Out-of-sample: Compare forecasted `yhat` to known true value `y`
...
# `ModelA`: Auto-impute Missing Values
...
with mdla:
    ...
    ida.extend(
        pm.sample_posterior_predictive(
            trace=ida.posterior, var_names=RVS_PPC
        ),
        join="right",
    )
```
...
### Compare Log-Likelihood vs Other Models
...
### Hierarchical values for missing data in $\mathbb{x}_{k}$
...
### View auto-imputed values of missing data in $x_{k}$ (`xk_unobserved`)
...
### In-sample: Compare auto-imputed values $x_{k}$ `xk_unobserved` to known true values
...
## Create PPC Forecast on `dfx_holdout` set
...
### Firstly: Rebuild model entirely, using `dfx_holdout`
...
### Secondly: sample PPC for missing values `xk_unobserved` in out-of-sample dataset
...
# Drop training-specific variables whose shapes are incompatible with holdout model
# (xk_unobserved has 26 elements in training vs 6 in holdout, xk is 30x2 vs 10x2)
ida_h.posterior = ida_h.posterior.drop_vars(["xk_unobserved", "xk"])
ida_h
```

```{python}
# Sample xk_unobserved, xk, and yhat together in a single call.
# Since xk_unobserved and xk were dropped from the posterior (incompatible shapes),
# they will be freshly sampled/computed for the holdout model's structure.
with mdla_h:
    ida_h = pm.sample_posterior_predictive(
        trace=ida_h.posterior,
        var_names=["xk_unobserved", "xk", "yhat"],
        predictions=True,
    )
ida_h
```

### Thirdly: sub the predictions for `xk_unobserved` into idata and sample PPC for `yhat`

```{python}
# All predictions (xk_unobserved, xk, yhat) were sampled in the previous cell.
# ida_h.predictions now contains all three variables.
ida_h
```
...
### Out-of-sample: Compare auto-imputed values $x_{k}$ `xk_unobserved` to known true values
...
### Out-of-sample: Compare forecasted $\hat{y}$ `yhat` to known true values
