...
- **Imputation**: fill the missing values with reasonable values that are based on the present values
  - often necessary and beneficial for accurate estimation
...
#### Simulate data where treatment effects outcome linearly
...
When the association between student ability $S$ and homework score $H$ is linear, we can still get fairly similar linear fits from the total and incomplete sample, you just lose precision on one the upper end of the $S$ dimensions in this simulation

#### Simulate data where treatment effects outcome non-linearly
...
plot_dog_homework(
    ...
    H,
    Hstar,
    title="Missing data based on common cause\nvery badly for non-linear relationships (common)",
)
```

- When the association between student ability $S$ and homework score $H$ is nonlinear, we now get very different linear fits from the total and incomplete sample
...
1. Data loss is **random and independent of causes**
...
5. Data loss is **conditioned on the outcome**
   - "Dog eats homework based on the score of the homework"
...
utils.draw_causal_graph(
    ...
    edge_props={
        ("u", "mG"): {"color": "blue", "fontcolor": "blue", "label": "anthro-narcissism"},
...
- All the colored arrows, and assumptions/hypotheses, are potentially in play
  - <span style="color:blue">**anthro-narcissism**: humans are more likely to study primates that are like them</span>
...
The equivalent causal-sub-graph used to model Brain Size in the "Phylogenetic Regression" section of [Lecture 16 - Gaussian Processes](<Lecture 16 - Gaussian Processes.ipynb>)
...
We can use a similar approach to simultaneously formulate parallel models for the other variables $G,M$ that may have missing values, a' la Full Luxury Bayes.
...
3. Impute $G$ using a $G$-specific sub-model
4. Impute $B,G,M$ using a sub-model for each

### 1. Ignore cases with missing $B$ values

#### Filter out observations missing brain volume measurements
...
#### Cross-tabulate missingness for $G,M$ on the resulting dataset
...
#### Fit the imputation model that ignores the sub-models for $G,M$
...
def plot_posterior_imputation(
    ...
    # Plot observed
    utils.plot_scatter(
        plot_data[xvar], plot_data[yvar], color="k", label="observed"
    )
    ...
    if impute_x and f"{xvar}_unobserved" in inference.posterior:
        ...
        impute_x_values = az.summary(
            inference, var_names=[f"{xvar}_unobserved"]
        )["mean"].values

    if impute_y and f"{yvar}_unobserved" in inference.posterior:
        y_impute_idx = plot_data[plot_data[yvar].isnull()].index.values
        impute_y_values = az.summary(
            inference, var_names=[f"{yvar}_unobserved"]
        )["mean"].values
...
plot_posterior_imputation(
    "M", "B", naive_imputation_inference, impute_x=True, title="Naively Imputed $M$ values with $B$"
)
```
...
Here we'll fit a complete-cases-only model where there's no need for imputation, but we throw away data.
...
### 3. Impute $G$ using a $G$-specific sub-models
...
2) Model that only includes Social group phylogenetic interactions
...
- However, by ignoring phylogenetic similarity, we aren't able to capture non-linearities in the data
...
we also get very similar effects to the phylogeny + $M \rightarrow G$ model

## Review: Imputing Primates

- **Key Idea**: missing values have probability distributions
- Think like a graph, not like a regression
- Blind imputation without relationships among predictors may be risky
